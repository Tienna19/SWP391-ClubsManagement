...
public class ClubDAO extends DBContext {
     ...
     *
     * @param club Club object with updated info
     ...
     * @return true if successful, false otherwise
     */
    public boolean updateClubStatus(int clubId, String status) {
        if (connection == null) {
            System.err.println("Database connection is null. Cannot update club status.");
            return false;
        }
        ...
        try {
            ...
            if (rowsAffected > 0) {
                System.out.println("[ClubDAO] Club status updated: ID=" + clubId + ", Status=" + status);
                return true;
            } else {
                System.err.println("[ClubDAO] No club found with ID=" + clubId);
                return false;
            }

        } catch (Exception e) {
            System.err.println("[ClubDAO] Error updating club status: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
    ...
    /**
     * Delete club from database (hard delete) WARNING: This will permanently
     * delete the club and may fail if there are foreign key constraints
     * Consider using updateClubStatus to set status='Deleted' instead (soft
     * delete)
     *
     * @param clubId Club ID
     * @return true if successful, false otherwise
